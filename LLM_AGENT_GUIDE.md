...
- Express routes: [server/routes/](server/routes/) (`auth.js`, `discord.js`, `stocks.js`, `turkish.js`, `nostalgiabait.js`)
- Server modules (top level): `db.js`, `currency.js`, `identity.js`, `discord-auth.js`, `stock-game.js`, `stock-price-cache.js`, `stock-providers/`, `character-store.js`, `lol-betting.js`, `lol-match-checker.js`, `riot-api.js`, `room-manager.js`, `game-logic.js`, `socket-utils.js`, `portfolio-history.js`, `pictochat-store.js`, `tierlist-store.js`, `food-leaderboards-store.js`, `food-ratings-store.js`, `turkish-lessons.js`, `turkish-streaks.js`, `brain-leaderboards.js`, `achievements.js`, `activity-feed.js`, `daily-streak.js`, `keep-alive.js`, `log-buffer.js`, `cleanup.js`, `sql/`
...
**Client:**
- Public pages: [public/](public/) (`index.html`, `login.html`, `discord.html`, `contacts.html`, `shop.html`, `achievements.html`, `nostalgiabait/`)
- Vendor: [public/vendor/discord-embedded-app-sdk.js](public/vendor/discord-embedded-app-sdk.js) — self-hosted IIFE bundle of `@discord/embedded-app-sdk` (this project has no client bundler, so it's pre-built and committed rather than built at deploy time; see "Discord Activity" below for how to rebuild it)
...
- Discord Activity users bypass `SITE_PASSWORD` via verified Discord OAuth instead — see "Discord Activity" below

**Discord Activity:**

The app can be launched inside Discord (voice channel → Activities) instead of visited as a website — everyone in the channel opens the same iframed instance. This bypasses `SITE_PASSWORD` (Discord OAuth replaces it) but does **not** change how players/wallets work: a Discord user still picks/claims a StrictHotel name the normal way (Trust-On-First-Use, see above); the only thing Discord auth does is skip the password gate and prefill that name field with the player's Discord display name.

- Flow: Discord launches the root URL with `?frame_id=...` appended → [server/routes/auth.js](server/routes/auth.js) `authMiddleware` detects `frame_id` on an unauthenticated request and redirects to `/discord.html` (preserving the query string) instead of `/login.html` → [public/discord.html](public/discord.html) runs the OAuth handshake via `@discord/embedded-app-sdk` → POSTs the resulting `code` to `/api/discord/session` ([server/routes/discord.js](server/routes/discord.js), token exchange + verified `/users/@me` lookup in [server/discord-auth.js](server/discord-auth.js)) → sets `req.session.authenticated = true` (same flag the password login uses) → redirects to `/`, which now passes the auth middleware normally.
- **Discord's proxy quirk**: from inside the activity iframe, JS-initiated `fetch()`/Socket.IO calls to our own backend must go through the fixed `/.proxy/` alias (a static `<script src>` load doesn't need this — only dynamic calls do; see the SDK's own README example). [shared/js/socket-init.js](shared/js/socket-init.js) patches the global `io` and `fetch` once, at the top of the file, so every game's existing `io()`/`fetch('/api/...')` call sites get this transparently — **don't** hand-add `/.proxy/` prefixes elsewhere; extend that one patch instead if a new dynamic-request pattern needs covering. `discord.html` itself isn't covered by that patch (it doesn't load socket-init.js) and hardcodes the `/.proxy/` prefix on its one fetch call directly.
- CSP: [server/index.js](server/index.js) sets a `frame-ancestors` header allowing `discord.com`/`discordsays.com` to iframe the app — this is the only CSP directive set anywhere in the app.
- The Embedded App SDK ships no browser bundle usable via a plain `<script>` tag and this project has no client bundler, so it's pre-built with esbuild (`--bundle --format=iife --global-name=DiscordEmbeddedAppSdk --minify`) and committed as [public/vendor/discord-embedded-app-sdk.js](public/vendor/discord-embedded-app-sdk.js). Rebuild it the same way after bumping the `@discord/embedded-app-sdk` version.
- **Setup that only the maintainer can do** (Discord Developer Portal, not something this repo can automate): create an application at <https://discord.com/developers/applications>, enable Activities for it, set a URL Mapping with prefix `/` targeting this server's public URL (the Render URL, or a tunnel in dev), and put its Client ID/Secret into `DISCORD_CLIENT_ID`/`DISCORD_CLIENT_SECRET`. Without both env vars set, `/api/discord/session` responds `503` and Discord users falls through to the normal password gate.
- Not yet built: mapping "everyone in the same Discord voice channel" to the same game room automatically. The Activity SDK exposes `discordSdk.instanceId`/`channelId` for this, and [server/room-manager.js](server/room-manager.js)'s existing room-code join model is the natural hook point, but no game currently auto-joins by instance — players still share a manual room code like on the website.
